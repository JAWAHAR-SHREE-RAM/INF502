...
[PA1 CODE](https://github.com/JAWAHAR-SHREE-RAM/INF502/blob/main/CODE/PA/1.py)

My Approach : Explanation

```
* I have used a total of 9 functions.
* Fucntion main():
       - Here is my program starts. 
       - I get the input from teh user for file name and read the sequences from the file. 
       - Then printing the sequences which I have read from the file.
       - Calling the options function for user to chosee between the no.of.matches and Maximum chain.

* Function option():
       - Getting the user's option and proceeding correspondingly.
       - Calling the engine function which is where all my operation begins.

* Function engine():
       - Here I have two function calls which returns the output to compare and print the final output.
       - Which is the sequences with higest no.of.matches and Maximum continues matches.
       - This Function prints the final output for both options.

* Function sequence1_shift():
       - This function is used to perform the sequence shifts on sequence A.

* Function sequence2_shift():
       - This function is used to perform the sequence shifts on sequence B.
       
* Function seq_A():
       - This function is used to find the shifted sequences with highest number of matches when sequence A is shifted.

* Function seq_B():
       - This function is used to find the shifted sequences with highest number of matches when sequence B is shifted.

*Function compare_continous():
       - This function is used to calculate the highest number of continues matches in a shifted sequences A and B.

*Function sequence_compare():
       - This function is used to find the no of matches in the shifted sequences A and B.
...
```
/Users/ram/Desktop/pycharm/venv/bin/python /Users/ram/Desktop/pycharm/list.py 
Enter the filename of the sequences 
1.txt
Enter the filename of the sequences 
2.txt
The sequences present in the file are :
A C T G A T C A C 
T T A G C T C G A 
*******SELECT AN OPTION*******
1. Number of matches
2. Maximum chain
2
Enter the number shifts to be performed3
The sequence with greatest continues matches are :
Sequence A ->  - - - A C T G A T C A C 
                       | |             
Sequence B ->  T T A G C T C G A - - - 
we obtain this maximum continues matches by shifting sequence A by 3 positions
There are 2 continues characters in both the sequence

Do you want to go back to options menu 'Y' or 'N' 
y
...
1
Enter the number shifts to be performed3
The sequence matches with greatest score are :
Sequence A ->  A C T G A T C A C - - 
                   |   |   |   |     
Sequence B ->  - - T T A G C T C G A 
we obtain this maximum number matches by shifting sequence B by 2 positions
The score is 4
...
Enter the number shifts to be performed3
The sequence matches with greatest score are :
Sequence A ->  A T T G A T C A C - 
                 | |   | | |       
Sequence B ->  - T T A A T C C G A 
we obtain this maximum number matches by shifting sequence B by 1 positions
The score is 5
...
2. Maximum chain
2
Enter the number shifts to be performed5
The sequence with greatest continues matches are:
Sequence A ->  A T T G A T C A C - 
                 | |   | | |       
Sequence B ->  - T T A A T C C G A 
we obtain this maximum continues matches by shifting sequence B by 1 positions
There are 3 maximum continues characters in both the sequence
